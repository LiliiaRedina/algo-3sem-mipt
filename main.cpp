...
void add_in_trie(std::vector<node>& trie, const std::string& str) {
    ...
    for (int i = 0; i < str.size(); ++i) {
        if (trie[vert].to[str[i] - 48] == -1) {
            trie.push_back(node());
            trie[vert].to[str[i] - 48] = int(trie.size()) - 1;
        }
        vert = trie[vert].to[str[i] - 48];
    }
    trie[vert].is_term = true;
}
...
void aho_corasick(std::vector<node>& trie) {
    ...
    while (!q.empty()) {
        ...
        for (int ch = 0; ch < 2; ++ch) {
            int son_vert_ch = trie[vert].to[ch];
            if (trie[son_vert_ch].link != -1) {
                continue;
            }
            trie[son_vert_ch].link = (vert == 0 ? 0 : trie[trie[vert].link].to[ch]);

            for (int ch_for_son = 0; ch_for_son < 2; ++ch_for_son) {
                if (trie[son_vert_ch].to[ch_for_son] != -1) {
                    continue;
                }
                trie[son_vert_ch].to[ch_for_son] = trie[trie[son_vert_ch].link].to[ch_for_son];
            }
